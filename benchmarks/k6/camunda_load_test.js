...
export function setup() {
  const baseUrl = __ENV.CAMUNDA_URL || 'http://localhost:8080';
  
  console.log(`Fetching decision definitions from ${baseUrl}...`);
  
  const res = http.get(`${baseUrl}/engine-rest/decision-definition`);
...
export default function(data) {
  ...
  const url = `${data.baseUrl}/engine-rest/decision-definition/key/${randomKey}/evaluate`;
